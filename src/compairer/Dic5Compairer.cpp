...
bool FiveCardsStrength::load(const char* file_path) {
    ...
    assert(flush_map.size() == (size_t)cnt);
    file.read(p_cnt, size_int);// 读取行数
    for (int i = 0; i < cnt; i++) {
        file.read(p_key, size_key);
        file.read(p_val, size_int);
    ...
    assert(other_map.size() == (size_t)cnt);
    file.close();
    return true;
}
...
bool FiveCardsStrength::check(unordered_map<uint64_t, int>& strength_map) {
    ...
    for (; it != it_end; it++, cnt++) {
        ...
        if (val1 != val2) {
            printf("%llx,%llx:%d != %d\ncheck failed!\n", (unsigned long long)key, (unsigned long long)ranks_hash(key), val1, val2);
            return false;
        }
    }
    printf("%d pass!\n", cnt);
...
Dic5Compairer::Dic5Compairer(string dic_dir,int lines, string dic_dir_bin):Compairer(std::move(dic_dir),lines){
    // 尝试加载 bin 文件，如果成功则直接返回
    if(fcs.load(dic_dir_bin.c_str())) {
        cout << "Loaded compairer from bin file: " << dic_dir_bin << endl;
        return;
    }
    
    // 否则从 txt 文件加载
    cout << "Loading compairer from txt file: " << this->dic_dir << endl;
